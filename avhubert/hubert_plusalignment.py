...
if DBG:
    from hubert_pretraining_plusalignment import (
        AVHubertPretrainingConfigPlusAlignment,
        AVHubertPretrainingTaskPlusAlignment,
    )
...
max_shift_num=25
@dataclass
class AVHubertConfigPlusAlignment(FairseqDataclass):
...
@register_model("av_hubert_plusalignment_model", dataclass=AVHubertConfigPlusAlignment)
class AVHubertModelplusalignment(BaseFairseqModel):
    def __init__(
        self,
        cfg: AVHubertConfigPlusAlignment,
        task_cfg: AVHubertPretrainingConfigPlusAlignment,
        ...
        **kwargs
        ...
        self.mask_emb = nn.Parameter(
            torch.FloatTensor(cfg.audio_feat_dim).uniform_() if self.masking_type == 'input' else torch.FloatTensor(cfg.encoder_embed_dim).uniform_() #uniform_()从0, to=1,随机取值
        )
    ...
    @classmethod
    def build_model(cls, cfg: AVHubertConfigPlusAlignment, task: AVHubertPretrainingTaskPlusAlignment):
        """Build a new model instance."""

        kwargs = {}
        model = AVHubertModelplusalignment(cfg, task.cfg, task.dictionaries, **kwargs)
        return model

    def apply_input_mask(self, x, padding_mask, target_list): #mask audio and video
        B, C, T = x.shape[:3]
    ...
    def forward(
        ...
        features_audio = self.forward_features(src_audio_origin, modality='audio') # features: [B, F, T]
        features_video = self.forward_features(src_video_origin, modality='video')

        # dealing with audio-video alignment
        shift_num = source['shift_num']
        features_audio_origin=features_audio
        #audio
        if self.modality_fuse == 'concat':
            features_audio_a = torch.cat([features_audio, 0 * features_video], dim=1)
        elif self.modality_fuse == 'add':
            features_audio_a = features_audio + 0 * features_video


        features_audio_a = features_audio_a.transpose(1, 2)
        features_audio_a = self.layer_norm(features_audio_a)


        if self.post_extract_proj is not None:
            features_audio_a = self.post_extract_proj(features_audio_a)

        features_audio_a = self.dropout_input(features_audio_a)

        x = features_audio_a
        x_audio, _ = self.encoder(
            x,
            padding_mask=None,
        ...
        #video
        if self.modality_fuse == 'concat':
            features_video_a = torch.cat([0 * features_audio_origin, features_video], dim=1)
        elif self.modality_fuse == 'add':
            features_video_a = 0*features_audio_origin + features_video

        features_video_a = features_video_a.transpose(1, 2)
        features_video_a = self.layer_norm(features_video_a)


        if self.post_extract_proj is not None:
            features_video_a = self.post_extract_proj(features_video_a)

        features_video_a = self.dropout_input(features_video_a)
        x = features_video_a
        x_video, _ = self.encoder(
            x,
            padding_mask=None,
            layer=None if output_layer is None else output_layer - 1
        )
        ...
        features_video_new=features_video.clone()

        #shift video
        for i in range(0,len(features_video)):
            inputv=features_video[i]
            inputv=torch.roll(inputv,-shift_res_batch[i],dims=1)
            features_video_new[i]=inputv
        features_video=features_video_new
        ...
        #print(features_audio.shape,features_video.shape,features.shape)
        ...
        result = {
            ...
            "features_audio": x_audio,
            "features_video": x_video,
            "shift_num": shift_num,
    ...
    def extract_finetune_shift(self, source, padding_mask=None, mask=False, ret_conv=False, output_layer=None):
        ...
        #audio is not need here since it is for lip-reading task, so that it's easier to compare performance with original avhubert
        features_audio = features_video.new_zeros(features_video.size(0), self.encoder_embed_dim, features_video.size(-1))
##
        features_video_new=features_video.clone()
        for i in range(0,len(features_video)):
...
##
    ...
    #for evaluating accuracy of predicting shift number
