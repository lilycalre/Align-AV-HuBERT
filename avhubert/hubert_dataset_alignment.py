    ...
    with open(manifest_path) as f:
        ...
        for ind, line in enumerate(f):
            ...
            if min_keep is not None and sz < min_keep:
                ...
                shifts.append(shift_list[0])
                sizes.append(sz)
                inds.append(ind*shift_time)#index

    if shift_time>0:
        tot = ind + 1 #for testing infer_alignment
    else:
        tot=ind+1
    logger.info(
        (
...
class AVHubertDataset(FairseqDataset):
    def __init__(
            ...
            label_rates: Union[List[float], float],  # -1 for sequence labels   self.cfg.label_rate
            pad_list: List[str],
            eos_list: List[str],
    ...
    def __getitem__(self, index):
        ...
        shift_num=self.shifts[index] 

        video_feats = self.shiftvideo(video_feats=video_feats, shift_num=shift_num)

        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "shift_num": shift_num}
    ...
    def collater(self, samples):
        ...
        if video_source is not None:
            collated_videos, padding_mask, audio_starts = self.collater_audio(video_source, audio_size, audio_starts) #对视频处理
        else:
