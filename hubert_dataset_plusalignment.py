...
def load_audio_visual(manifest_path, max_keep, min_keep, frame_rate, label_paths, label_rates, max_shift_num,shift_time,tol=0.1):
    ...
    with open(manifest_path) as f:
        ...
        for ind, line in enumerate(f):
            ...
            if min_keep is not None and sz < min_keep:
                ...
                #shift number
                shifts.append(0)
                list1 = range(-min(sz-2,int(max_shift_num*frame_rate)),min(sz-2,int(max_shift_num*frame_rate))+1)  #
                shift_list = random.sample(list1, shift_time)
                inds.append(ind)
                sizes.append(sz)
    ...
    return root, names, inds, tot, sizes, shifts

def load_label(label_path, inds, tot, shift_time):
    with open(label_path) as f:
        labels = [line.rstrip() for line in f]
        assert (
            len(labels)*(shift_time + 1) == tot
        ), f"number of labels does not match ({len(labels)*(shift_time + 1)} != {tot})"
        labels_output=[]
        for i in inds:
            label=labels[i]
            for j in range(shift_time+1):
...
def load_label_offset(label_path, inds, tot,shift_time):
    with open(label_path) as f:
        code_lengths = [len(line.encode("utf-8")) for line in f]
        assert (
            len(code_lengths)*(shift_time + 1) == tot
        ), f"number of labels does not match ({len(code_lengths)*(shift_time + 1)} != {tot})"
        offsets = list(itertools.accumulate([0] + code_lengths))
        offsets_output=[]
        for i in inds:
            offset=(offsets[i], offsets[i + 1])
            for j in range(shift_time+1):
                offsets_output.append(offset)
...
def verify_label_lengths(
    ...
    with open(label_path) as f:
        ...
        assert len(lengths)*(shift_time+1) == tot
        lengths_output=[]
        for i in inds:
            length=lengths[i]
            for j in range(shift_time+1):
                lengths_output.append(length)
        lengths=lengths_output
    num_invalid = 0
    for i, ind in enumerate(inds):
        for j in range(shift_time):
            dur_from_audio = audio_sizes[i+j] / audio_rate
            dur_from_label = lengths[i+j] / label_rate
            if abs(dur_from_audio - dur_from_label) > tol:
                logger.warning(
                    (
                        ...
                        f"num. of samples = {audio_sizes[i+j]}; "
                        f"label length = {lengths[i+j]}"
                    )
                )
                num_invalid += 1
    if num_invalid > 0:
...
class AVHubertDataset(FairseqDataset):
    def __init__(
            ...
            noise_snr=0,
            noise_num=1,
            max_shift_num=1.0, #max shift number (s)
            shift_time = 3, #expand dataset with unaligned audio-video pairs of (original size * shift_time)
    ):
        self.label_rates = (
        ...
        self.audio_root, self.names, inds, tot, self.sizes, self.shifts = load_audio_visual(manifest_path, max_keep_sample_size, min_keep_sample_size, frame_rate=sample_rate, label_paths=label_paths, label_rates=self.label_rates, max_shift_num=max_shift_num,shift_time=shift_time)
        ...
        if store_labels:
            self.label_list = [load_label(p, inds, tot, shift_time) for p in label_paths]
        else:
            self.label_paths = label_paths
            self.label_offsets_list = [
                load_label_offset(p, inds, tot, shift_time) for p in label_paths
            ]
        assert (
        ...
        if not skip_verify:
            for label_path, label_rate in zip(label_paths, self.label_rates):
                verify_label_lengths(self.sizes, self.sample_rate, label_path, label_rate, inds, tot, shift_time)
        else:
        ...
        from .utils import HorizontalFlip
        if image_aug:
    ...
    def load_video(self, audio_name):
        ...
        feats = load_video(os.path.join(self.audio_root, audio_name))
    ...
    def __getitem__(self, index):
        ...
        fid = self.names[index][1].split(':')[1]#names: (video_path, audio_path+':'+audio_id)
        shift_num=self.shifts[index]  #shift number
        video_feats = self.shiftvideo(video_feats=video_feats, shift_num=shift_num)

        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels, "shift_num": shift_num}

    def __len__(self):
        return len(self.sizes)

    def shiftvideo(self,video_feats, shift_num):
        #shift_num > 0 ，video is slower than audio
        #shift_num < 0 ，video is faster than audio
        video_feats=torch.roll(video_feats, shift_num, dims=0)
        return video_feats
